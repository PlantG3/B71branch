nucmer.twosets.plot.modified<-function(datapath=".", datafile, lend.turnoff=F,
                                       aln.min.size=100, aln.min.identity=90, chrset.first=c(),chrset.second=c(),
  ...
  if (length(chrset.first)>0){
    co<-co[co$first %in% chrset.first, ]
  }
  if (length(chrset.second)>0){
    co<-co[co$second %in% chrset.second, ]
  }
