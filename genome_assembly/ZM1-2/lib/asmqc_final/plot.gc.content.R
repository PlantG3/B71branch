...
#chrsize, GC counts, read counts, Cent, Telomeres, filename, rdna
args
...
outpre<-args[6]
rdna<-read.delim(args[7],header = F)
##
...
#rdna<-read.delim("/bulk/guifanglin/1-projects/2-wheatblast/1-TF05-1/1-asm/cache/4_np_reorganize/1ac_TF051mc7sp1np2020/6o.1ac_TF051mc7sp1np2020.ro.1.rdnaRT.min500bp.bed",header=F)
#outpre<-"1ac_TF051mc7sp1np2020.ro.1"
  
  #GC content
pdf(paste0(outpre,".gc.pdf"), width=8, height=8) #width=12
...
plot(NULL, NULL, ylim = yrange, xlim = xrange,
     ...
     bty = "n")
 # legend(max(chrsize$V2)-3000000,3.5*2, legend=c("centromere", "telomeres","18srDNA unit"),
 #      col=c("red", "blue","purple"), pch =c(23,25,2), cex=0.8)
legend(max(chrsize$V2)-3000000,3.5*2, legend=c( "telomeres"),
       col=c("blue"), pch =c(25), cex=0.8)
s<-nrow(chrsize)*3.5
...
for (i in 1:nrow(chrsize)){
  ...
  axis(2, at=s-1,labels =lab,cex=0.1, las=1)
  s<-s-3.5
}
