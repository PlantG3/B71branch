...
#source("../../../4_reorganize/utils/nucmer.twosets.plot.modified.R")
nucmer.twosets.plot.modified<-function(datapath=".", datafile, lend.turnoff=F,
                              aln.min.size=100, aln.min.identity=90,
                              xlabel.rm = "chr",xlabel.rm2="test", ylabel.rm = "chr",ylabel.rm2 = "test",
                              tableout=F, pdfout=F, outpath=".", line.width.factor=5,
                              xrange=NULL, yrange=NULL,
                              pdf.width = 10, pdf.height = 10,
                              tableoutfile="seq.nucmer.txt", imageoutfile="seq.nucmer.pdf", toremove.first=NA, toremove.second=NA,xlab="genome A",ylab="genome B",...) {
  ...
  sc.cmd <- paste("/homes/liu3zhen/local/bin/show-coords -clT -q -L", aln.min.size, "-I",
  ...
  ### plot
  plot(NULL, NULL, type="n", xlim=xrange, ylim=yrange,xlab =xlab, ylab=ylab, ...)
  ...
  xlabels <- gsub(xlabel.rm2, "", xlabels)
  ylabels <- names(second.accum)
  cat("--", ylabels, "\n")
  ylabels <- gsub(ylabel.rm, "", ylabels)
  ylabels <- gsub(ylabel.rm2, "", ylabels)
  cat("==", ylabels, "\n")
  text(x=first.coord, y= - max.ysize / 50, labels=xlabels, cex = 0.5, xpd = T)
  text(x= - max.xsize / 40, y=second.coord, labels=ylabels, cex = 0.5, xpd = T)
  if (pdfout) {
    dev.off()
  }
}
...
cat(deltafile, "\n")
comp<-gsub("1o.","",deltafile)
comp<-gsub(".delta","",comp)
x<-paste0(gsub("\\.2\\..*","",comp))
y<-paste0(gsub(".*\\.2\\.","",comp))

min.match<-10000
min.ID<-99

nucmer.twosets.plot.modified(datapath=datapath, datafile=deltafile,
                               lend.turnoff=T,
                               aln.min.size=min.match, aln.min.identity=min.ID,
                               xlab = x, ylab = y,
                               main = paste0(x, " vs. ", y),
				line.width.factor=2, xlabel.rm = "tig0000", 
				ylabel.rm= "tig0000", tableout=T, pdfout=T,
				 outpath=".",pdf.width = 4.9, pdf.height = 5,
                               tableoutfile=paste0("2o-",x,"vs",y,".all.txt"),
                               imageoutfile=paste0("2o-",x,"vs",y,".all.pdf"))
...
nucmer.twosets.plot.modified(datapath=datapath, datafile=deltafile,
                    ...
                    xlab = x, ylab = y,
                    main = paste0(x, " vs. ", y),
                    line.width.factor=2, xlabel.rm = "tig00000", ylabel.rm = "tig000
00",
                    tableout=T, pdfout=T, outpath=".",
                    pdf.width = 4.9, pdf.height = 5,
                    tableoutfile=paste0("2o-",x,"vs",y,"nocore.txt"),
                    imageoutfile=paste0("2o-",x,"vs",y,"nocore.pdf"),toremove.second  = c(

"chr1","chr2","chr3","chr4","chr5","chr6","chr7"),toremove.first  = c(
