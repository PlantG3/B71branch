...
pdf(paste0("6o-",outpre,"read.counts.pdf"), width=12, height=8)
yrange=c(0, (nrow(chrsize)*3.5))
xrange=c(0, max(chrsize$V2))
ylab.text="reads count"
#chrsize<-chrsize[chrsize$V1 %in% c("tig00000008","tig00000011","tig00000012","tig00000014","tig00000015"),]
#indata<-indata[indata$V1 %in% c("tig00000008","tig00000011","tig00000012","tig00000014","tig00000015"), ]
plot(NULL, NULL, ylim = yrange, xlim = xrange,
     xlab = "physical coordinate (bp)", ylab = ylab.text, yaxt="n",
     main = paste0(outpre,"read counts"), 
     bty = "n")
...
for (i in 1:nrow(chrsize)){
  ...
  rect(0,s-3,chrsize[i,2],s,border = T)
  #lines(x=c(0,chrsize[i,2]),y=c(s-3*0.3,s-3*0.3),lty=2,cex=0.01)
  lines(x=c(0,chrsize[i,2]),y=c(s-3+3/ymax*ymd,s-3+3/ymax*ymd),lty=2,cex=0.01)
  axis(2, at=s-1,labels =lab)
  s<-s-3.5
}
...
for (i in 1:nrow(chrsize)){
  a<-readc[readc$V1==chrsize[i,1],]
  #points(x=a$V2+150,y=s-3+a$V4*3*0.01,cex=0.01,col="grey50")
  for (j in 1:(nrow(a)-1)){
    lines(x=c(a[j,2],a[j+1,2]),y=c(s-3+3/ymax*a[j,4],s-3+3/ymax*a[j+1,4]),cex=0.01,col="grey30")
  }
