...
my $ver = "version: 0.1.1";

my ($mem, $time, $bampaths, $otherpara, $help);
my ($threads, $checkscript, $version, $gatkpath);
...
if (!defined $gatkpath) {
	$gatkpath = "/homes/liu3zhen/software/GATK/gatk4/gatk-4.1.0.0";
}
...
&prompt if ($help);
$otherpara = "" if !defined $otherpara;
...
if (defined $otherpara) {
	print GATK " $otherpara ";
}
print GATK "\n";
...
print MERGE "$gatk MergeVcfs --java-options \'-Xmx$merge_mem\' -I $vcflist -O $finalvcf\n";
close MERGE;
...
sub prompt {
	...
	--gatkpath <gatk path>: GATK excutable path; default=/homes/liu3zhen/software/GATK/gatk4/gatk-4.1.0.0/gatk
	--maxlen <max length>: maximal interval length of each job to call variants; default=2000000
	--otherpara <str>: other parameter for HaplotypeCaller, e.g., "--sample-ploidy 1"; no other parameters by default
